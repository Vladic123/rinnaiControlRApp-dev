...
class CountDown extends React.Component {
    ...
    renderDigit = (d, type) => {
        ...
        return (
            <View style={[styles.digitCont, {width: size + 2, height: size + 2}, digitStyle]}>
                <Text
                    style={[
                        styles.digitTxt,
                        {fontSize: type === 'second' ? 10 : 12},
                        {color: type === 'second' ? '#878585' : '#000000'},
                        digitTxtStyle,
                    ]}>
                    {d}
                </Text>
    ...
    renderDoubleDigits = (label, digits, type) => {
        return (
            <View style={styles.doubleDigitCont}>
                <View style={{...styles.timeInnerCont}}>{this.renderDigit(digits, type)}</View>
    ...
    renderCountDown = () => {
        ...
        return (
            <Component style={styles.timeCont} onPress={this.props.onPress}>
                {timeToShow.includes('D') ? this.renderDoubleDigits(timeLabels.d, newTime[0], 'day') : null}
                {showSeparator && timeToShow.includes('D') && timeToShow.includes('H') ? this.renderSeparator() : null}
                {timeToShow.includes('H') ? this.renderDoubleDigits(timeLabels.h, newTime[1], 'hour') : null}
                {showSeparator && timeToShow.includes('H') && timeToShow.includes('M') ? this.renderSeparator() : null}
                {timeToShow.includes('M') ? this.renderDoubleDigits(timeLabels.m, newTime[2], 'minute') : null}
                {showSeparator && timeToShow.includes('M') && timeToShow.includes('S') ? this.renderSeparator() : null}
                {timeToShow.includes('S') ? this.renderDoubleDigits(timeLabels.s, newTime[3], 'second') : null}
            </Component>
        );
    };
