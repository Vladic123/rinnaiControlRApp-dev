...
RCT_EXPORT_METHOD(scpUpload:(NSString *)localPath
    ...
    if (client) {
        NMSSHSession* session = client._session;
        if ([self isConnected:session withCallback:callback]) {
            BOOL result = [client scpUpload:localPath toPath:remotePath];
            if (result) {
                callback(@[]);
            } else {
                if (client._uploadContinue) {
                    NSLog(@"Error uploading file");
                    callback(@[[NSString stringWithFormat:@"Failed to upload %@ to %@", localPath, remotePath]]);
                } else {
                    callback(@[@"Upload canceled"]);
                }
            }
