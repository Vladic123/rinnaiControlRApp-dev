...
- (BOOL) scpUpload:(NSString *)localPath toPath:(NSString *)remotePath {
    NMSSHChannel *channel = _session.channel;
    channel.delegate = self;
    _uploadContinue = true;
    uploadedPerc = 0;
    long long fileSize = [[[NSFileManager defaultManager] attributesOfItemAtPath:localPath error:nil][NSFileSize] longLongValue];

    BOOL result = [channel uploadFile:localPath to:remotePath progress: ^BOOL (NSUInteger bytes) {
        int newPerc = (int)(100.0f * bytes / fileSize);
        if (newPerc % 5 == 0 && newPerc > uploadedPerc) {
            uploadedPerc = newPerc;
            [self.delegate uploadProgressEvent:uploadedPerc withKey:_key];
