...
public class RNSshClientModule extends ReactContextBaseJavaModule {
  ...
  @ReactMethod
  public void scpUpload(final String localFile, final String remotePath, final String key, final Callback callback) {
    new Thread(new Runnable() {
      public void run() {
        try {
          SSHClient client = clientPool.get(key);
...
//          map.putString("value", String.valueOf(charVal));
  ...
  @ReactMethod
  public void toggleCellData(Context context, final Boolean enabled) throws ClassNotFoundException, NoSuchFieldException, IllegalAccessException, NoSuchMethodException, InvocationTargetException{
