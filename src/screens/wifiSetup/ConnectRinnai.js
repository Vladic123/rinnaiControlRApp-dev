...
const ConnectRinnai = ({navigation}) => {
    ...
    async function getDeviceInformation() {
        ...
        try {
            setIsLoading(true);
            const metadata = await ModuleService.getModuleMetadata();
            const {deviceSerial, deviceFirmware, deviceAylaDSN} = metadata;
            const setPairing = {
            ...
            Alert.alert('Failed to get device details', 'Please check your wifi connection or try again', [{text: 'OK'}], {
                cancelable: false,
            });
        } finally {
            setIsLoading(false);
    ...
    const checkWifi = () => {
        WifiManager.getCurrentWifiSSID().then(
            (ssid) => {
                const isRinnaiNetwork = ssid.startsWith('Rinnai') || ssid.startsWith('rinnai');
                setWifi(ssid);
                if (isRinnaiNetwork) {
                    const update = {
                        bssid: connection.bssid,
